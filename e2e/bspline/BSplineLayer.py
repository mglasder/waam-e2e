...
class BSplineLayer(nn.Module):
    ...
    def __init__(
        ...
        if self.shared_weights:
            ...
            self.weight = Parameter(torch.Tensor(self.n_bases, self.in_features), requires_grad=True)
        if bias:
            self.bias = Parameter(
                torch.Tensor(
                    self.out_features,
    ...
    def forward(self, input):
        ...
        if self.weighted_sum:
            return self.shrunk_hidden_basis_layer(expanded_features, self.weight, self.bias)
        else:
    ...
    def expanded_hidden_basis_layer(self, input):
        ...
        if input.is_cuda:
            cuda = True
        expanded_features = EncodeSplines(n_bases=self.n_bases).fit_transform(input)  # need import that class
        if cuda:
