...
from e2e.bspline.BSpline import BSpline
...
def get_knots(start, end, n_bases=5, spline_order=3):
    ...
    knots = torch.linspace(start=start - dknots * (m + 1), end=end + dknots * (m + 1), steps=nk + 2 * m + 2)
    return knots.float()
...
def get_X_spline(x, knots, n_bases=5, spline_order=3, add_intercept=True):
    ...
    for i in range(n_bases):
        ...
        tck[1] = vec
        if cuda:
            X[:, i] = torch.from_numpy(si.splev(x, tck, der=0)).to("cuda")  # TODO: specify cuda number
        else:
