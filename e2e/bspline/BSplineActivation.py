...
class BSplineActivation(nn.Module):
    ...
    def __init__(
        ...
        if mode not in ["conv", "linear"]:
            raise ValueError('Mode should be either string "conv" or "linear".')
        if size % 2 == 0:
        ...
        self.spline_grid = spline_grid_from_range(self.spline_size[0], self.spline_range)
    ...
    def get_grid_tensor(self, size_, grid_):
        ...
        """
        grid_arange = torch.arange(-(size_ // 2), (size_ // 2) + 1).to(**self.device_type).mul(grid_)
    ...
    def reshape_forward(self, input):
        ...
        assert len(x.shape) == 4, "input to activation should be 2D or 3D or 4D."
    ...
    def forward(self, input):
        ...
        output = BSplineActivationFunc.apply(x, self.coefficients_vect_, self.grid, self.zero_knot_indexes, self.size)
