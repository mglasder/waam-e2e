...
class PredictionPlotting(Callback):
    ...
    def _plot_example(self, pred, trgt, inpt, id_, batch_loss, epoch, stage):
        ...
        fig, ax = plt.subplots()
        ax.plot(trgt_np[1, :], trgt_np[0, :], color="blue", label="target (after)")
        ax.plot(pred_np[1, :], pred_np[0, :], color="red", label="pred")
        ax.plot(inpt_np[1, :], inpt_np[0, :], color="black", label="input (before)")
